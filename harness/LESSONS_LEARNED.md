...
- **temp-stand 점유 vs 클레임 분리**: `_agent_occupies_temp_stand_slot`가 `temp_stand_id`만 세팅돼도 `occupied_by`를 채우면, `can_reserve_path`의 `temp_stand_busy` 체크가 incident edge(폴리라인상 이웃 taxiway edge) 전체를 닫아버려, **같은 택시웨이 선상에서 더 상류 temp stand를 쓰려는 다른 기체가 영구 고착**된다(예: node 70=T002 claim이 걸리면 node 68=T001로 가는 경로가 봉쇄). occupancy는 실제로 주기(`awaiting_apron_from_temp=True`) 상태일 때만 채우고, 픽 중복 방지는 `_temp_stand_has_other_claimant_or_occupant`의 `ag2.temp_stand_id` 스캔으로 충분하다.
- **splice 시 start 엔드포인트 스냅 필수**: 착륙 중 temp 우회를 삽입할 때 `_build_prep_xy_to_xy_phase(..., snap_exact_start_xy=False)` 이면 Dijkstra는 `start_xy`에서 가장 가까운 graph 노드(폴리라인상 이전 지점)로 스냅되어, temp_prep의 첫 micro-segment가 Landing 마지막 micro-segment와 **동일 구간을 중복 생성**한다. 이후 Landing→ArrTaxi_Temp 전환 틱에서 `_snap_agent_to_first_segment`가 기체를 중복 세그먼트의 p0로 끌어 당겨 **속도 급락·위치 역행(RTX 뚜둑거림)**이 발생. `snap_exact_end_xy`와 마찬가지로 **splice 경로에는 반드시 `snap_exact_start_xy=True`**를 함께 넘겨 이음새를 무결하게 만든다.
- **temp→apron 주입은 prior-hop reverse 예외가 필요**: temp stand에 주기한 뒤 목적 주기장으로 나가려면, temp 접근 때 마지막으로 탔던 taxiway/connector edge를 반대 방향으로 되짚어 나가는 것이 정상 egress일 수 있다. 일반 reroute의 U-turn 방지용 `_graph_path_has_disallowed_reverse_of_prior_hops`를 그대로 적용하면 `PARK-155`처럼 목적 주기장이 비어 있어도 `_build_prep_xy_to_xy_phase(..., Arr_taxi)`가 빈 경로로 실패한다. 예외는 `_try_inject_arr_taxi_from_temp_stand`의 temp→apron leg에만 opt-in으로 둔다.
