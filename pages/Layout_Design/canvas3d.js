  ...
  function buildFullAirsideTimelineForFlight(f) {
    ...
    const allSt = typeof allStandsForFlightAssignment === 'function' ? allStandsForFlightAssignment() : [];
    const standForClamp = (f.standId != null && f.standId !== '')
      ? allSt.find(function(s) { return s && s.id === f.standId; }) || null
      : null;
  ...
  /** CW↔CCW 전환 시 vertices 반전 → vertices[0]이 현재 direction 기준 시작점, lineup은 해당 모드 거리로 표시. Both는 Infra 슬롯으로 방향 제어하므로 반전하지 않음. */
  ...
  /** Runways whose lineup point (same geometry as red/green grid dot) is not near any runway_exit / runway_taxiway that touches the runway. */
  function getLayoutLineupDisconnectedFromRunwayExitLabels() {
    const lines = [];
    const tws = state.taxiways || [];
    for (let wi = 0; wi < tws.length; wi++) {
      const tw = tws[wi];
      ...
      const lp = _pointOnPolylineAtDistPxForLineup(rwPts, d);
      if (!lp) continue;
      if (isLineupPointTouchingRunwayTaxiwayOnRunway(tw, lp)) continue;
      let lab = '';
      if (typeof getRunwayDisplayLabelById === 'function') {
        const lx = getRunwayDisplayLabelById(tw.id);
        if (lx && lx !== '—') lab = lx;
      }
      if (!lab) lab = String(tw.name || '').trim() || (tw.id != null ? String(tw.id) : '—');
      lines.push(lab + ': line-up not adjacent to runway exit/runway taxiway');
    }
    return lines;
  }
  function hasLayoutLineupRunwayExitDisconnectedViolation() {
    return getLayoutLineupDisconnectedFromRunwayExitLabels().length > 0;
  }
  ...
  /** Runway 또는 활주 폴리선과 교차/접촉하는 runway_exit · runway_taxiway (Vertices 기준). */
  function taxiwayIsRunwayOrGeometricallyTouchesAnyRunway(tw) {
    if (!tw) return false;
    if (tw.pathType === 'runway') return true;
    if (tw.pathType !== 'runway_exit' && tw.pathType !== 'runway_taxiway') return false;
    const verts = tw.vertices;
    if (!Array.isArray(verts) || verts.length < 2) return false;
    const runways = (state.taxiways || []).filter(function(t) {
      return t && t.pathType === 'runway' && Array.isArray(t.vertices) && t.vertices.length >= 2;
    });
    if (!runways.length) return false;
    for (let ri = 0; ri < runways.length; ri++) {
      const rw = runways[ri];
      let rVerts = rw.vertices.map(function(v) { return [v.col, v.row]; });
      let ei = 0;
      for (ei = 0; ei < verts.length; ei++) {
        const q = [verts[ei].col, verts[ei].row];
        for (let i = 0; i < rVerts.length - 1; i++) {
          const a = rVerts[i], b = rVerts[i + 1];
          if (pointOnSegmentStrict(a, b, q)) return true;
        }
      }
      let ev = verts.map(function(v) { return [v.col, v.row]; });
      for (ei = 0; ei < ev.length - 1; ei++) {
        const ea = ev[ei], eb = ev[ei + 1];
        let i = 0;
        for (i = 0; i < rVerts.length - 1; i++) {
          const ra = rVerts[i], rb = rVerts[i + 1];
          if (segmentSegmentIntersection(ea, eb, ra, rb)) return true;
          const ovRw = collinearSegmentOverlapOnAB(ra, rb, ea, eb);
          if (ovRw) return true;
        }
      }
  ...
  function buildPathGraph(selectedArrRetId, runwayDirectionForExit, pathGraphOpts) {
    ...
    function registerDirectedEdge(fromIdx, toIdx, cost, rawDist, pts, linkId, pathType, pathDir, pathOpsTw) {
      ...
      if (pathOpsTw) attachNonDefaultPathOpsToSimEdge(edge, pathOpsTw);
      edges.push(edge);
    ...
    return {
      ...
      edges: g.edges.map(function(e) {
        ...
        if (typeof e.icaoCategoryAllowedMask === 'number' && isFinite(e.icaoCategoryAllowedMask)) {
          row.icaoCategoryAllowedMask = (e.icaoCategoryAllowedMask | 0) & ICAO_CAT_ALLOWED_MASK_FULL;
        }
        return row;
      }),
  ...
  function drawFlights2D() {
    ...
    state.flights.forEach(f => {
      ...
      if (isFlightSel) {
        ...
        ctx.lineWidth = Math.max(2.8, outlineWidth + 2.2);
        ctx.lineJoin = 'round';
        ctx.stroke();
        ctx.restore();
      } else if (outlineWidth > 0 && outlineColor) {
  ...
  (function initFlightUI() {
    ...
    if (globalUpdateBtn) {
      globalUpdateBtn.addEventListener('click', function() {
        ...
        const disconnectedLineup = typeof getLayoutLineupDisconnectedFromRunwayExitLabels === 'function'
          ? getLayoutLineupDisconnectedFromRunwayExitLabels() : [];
        if (disconnectedLineup.length) {
          failProSim('Line-up not connected to runway exit (red line-up marker on grid): ' + disconnectedLineup.join(' · '));
          return;
        }
        if (typeof markGlobalUpdateStale === 'function') markGlobalUpdateStale();
    ...
    function syncTableToFlightState() {
      ...
      rows.forEach(function(row) {
        ...
        const map = {
          sibtMin: flightSchedColIndex('sibt', k),
          sobtMin: flightSchedColIndex('sobt', k),
          eldtMin: flightSchedColIndex('eldt', k),
          eibtMin: flightSchedColIndex('eibt', k),
          eobtMin: flightSchedColIndex('eobt', k),
          etotMin: etotIdx
        };
  ...
  document.getElementById('btnTaxiwayDraw').addEventListener('click', function() {
    ...
    pushUndo();
    normalizePathOpsPayloadInPlaceForTaxiway(taxiway);
  ...
  function onLayoutContainerMouseUp(ev) {
    ...
    if (state.pathArcDrag) {
      ...
      if (typeof syncPanelFromState === 'function') syncPanelFromState();
      if (typeof updateObjectInfo === 'function') updateObjectInfo();
      if (typeof redrawLayoutAfterEdit === 'function') redrawLayoutAfterEdit(touchedTwForRedraw);
      else {
