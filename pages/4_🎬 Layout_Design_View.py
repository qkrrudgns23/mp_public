"""
Simulation Viewer — replays airside DES results on the full Layout Designer canvas.

Loads sim_output.json, converts DES positions {t, col, row} into flight timelines
{t, x, y} in pixel coordinates, injects them into the layout flights, then renders
the full Layout_Design canvas with playback enabled.
"""
import json
import logging
import os
...
from utils.layout_receiver import list_layout_names, start_layout_receiver
...
# --- load Information.json config values (same as Layout_Design.py) ---
...
_rw_exit_allowed_default_raw = _cfg_list(
    _rw_exit_ui, "allowedRwDirectionsDefault", ["clockwise", "counter_clockwise"],
)
...
# --- Load sim_output.json ---

sim_data: dict = {}
if _SIM_OUTPUT_FILE.is_file():
    try:
        sim_data = json.loads(_SIM_OUTPUT_FILE.read_text(encoding="utf-8"))
    except Exception:
        _logger.warning("Failed to load sim_output.json", exc_info=True)
...
layout_for_html = sim_data.get("layout", {})
ensure_flight_service_dates(layout_for_html, _default_flight_service_date)
cell_size = layout_for_html.get("grid", {}).get("cellSize", CELL_SIZE)

# --- Inject DES positions as _simTimeline into each flight ---

_positions = sim_data.get("positions", {})
_schedule = sim_data.get("schedule", [])
_sched_by_id = {s["flight_id"]: s for s in _schedule if isinstance(s, dict) and "flight_id" in s}

for f in layout_for_html.get("flights", []):
    fid = f.get("id", "")
    pts = _positions.get(fid, [])
    if pts:
        f["_simTimeline"] = [
            {"t": p["t"], "x": p["col"] * cell_size, "y": p["row"] * cell_size}
            for p in pts
        ]
    else:
        f["_simTimeline"] = []
    sched = _sched_by_id.get(fid)
    if sched:
        f["_simSchedule"] = {
            "eldtSec": sched.get("ALDT"),
            "eibtSec": sched.get("AIBT"),
            "eobtSec": sched.get("AOBT"),
            "etotSec": sched.get("ATOT"),
        }
    else:
        f["_simSchedule"] = {}


# --- Style / grid config (same as Layout_Design.py) ---
...
def _style_root_css_from_information(info: dict) -> str:
    ...
    lines: list[str] = [":root {"]
    ui = st_d.get("uiTheme") if isinstance(st_d.get("uiTheme"), dict) else {}
    _ui_map = [
    ...
    g = st_d.get("gantt") if isinstance(st_d.get("gantt"), dict) else {}
    c2 = st_d.get("canvas2d") if isinstance(st_d.get("canvas2d"), dict) else {}
    for name, val in [
...
# --- SVG icons (same as Layout_Design.py) ---
...
def _designer_js_config_dict() -> dict[str, object]:
    return {
        ...
        "layoutNames": list_layout_names(),
        "initialLayout": layout_for_html,
        "initialLayoutDisplayName": "sim_result",
...
# --- Post-init JS: inject DES timelines into flights and start playback ---
...
(function() {
  ...
  flights.forEach(function(f) {
    if (f._simTimeline && f._simTimeline.length) {
      f.timeline = f._simTimeline;
      f.timeline_meta = f._simSchedule || {};
      delete f._simTimeline;
      delete f._simSchedule;
    }
  });
...
def _build_viewer_html() -> str:
    ...
    css = _apply_designer_placeholders(css_tm, {
    ...
    body = _apply_designer_placeholders(body_tm, {
    ...
    for needle in (
        "__PANEL_FORM_", "__FLIGHT_SPEED_OPTIONS_HTML__", "__TOP_PANEL_TABS_HTML__",
        "__LAYOUT_MODE_TABS_", "__UI_G_", "__UI_TERM_", "__UI_TW_W__",
        "__UI_RW_", "__UI_EX_", "__UI_FLIGHT_", "__GRID_COLS__",
        "__GRID_ROWS__", "__CELL_SIZE__", "__UI_SHARED_",
    ):
        if needle in out:
...
st.markdown("""
  <style>
    .stApp, [data-testid="stAppViewContainer"], section.main { background-color: """ + _designer_background_base() + """ !important; }
