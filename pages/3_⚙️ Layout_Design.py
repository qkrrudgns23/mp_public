...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .flight-schedule-table .flight-col-s th {{ background:rgba(0,122,255,0.1); }}
    .flight-schedule-table .flight-col-s td {{ background:rgba(0,122,255,0.05); }}
    .flight-schedule-table .flight-col-s-start th {{ border-left:2px solid rgba(0,122,255,0.65); }}
    .flight-schedule-table .flight-col-s-start td {{ border-left:2px solid rgba(0,122,255,0.5); }}
    .flight-schedule-table .flight-col-sd th {{ background:rgba(34,197,94,0.1); }}
    .flight-schedule-table .flight-col-sd td {{ background:rgba(34,197,94,0.05); }}
    .flight-schedule-table .flight-col-sd-start th {{ border-left:2px solid rgba(34,197,94,0.65); }}
    .flight-schedule-table .flight-col-sd-start td {{ border-left:2px solid rgba(34,197,94,0.5); }}
    .flight-schedule-table .flight-col-e th {{ background:rgba(251,55,197,0.1); }}
    .flight-schedule-table .flight-col-e td {{ background:rgba(251,55,197,0.05); }}
    .flight-schedule-table .flight-col-e-start th {{ border-left:2px solid rgba(251,55,197,0.65); }}
    .flight-schedule-table .flight-col-e-start td {{ border-left:2px solid rgba(251,55,197,0.5); }}
    ...
    #allocationGantt {{ margin-top: 10px; padding: 10px 12px 10px 0; background: rgba(15,23,42,0.98); border-radius: 10px; border: 1px solid rgba(55,65,81,0.95); height: 800px; max-height: 800px; overflow: hidden; display: flex; flex-direction: column; }}
    .alloc-gantt-root {{ display:flex; align-items:stretch; width:100%; flex:1; min-height:0; }}
    .alloc-gantt-label-col {{ flex-shrink:0; width:122px; padding-left:12px; padding-right:8px; box-sizing:border-box; display:flex; flex-direction:column; overflow-y:auto; overflow-x:hidden; min-height:0; scrollbar-width:none; -ms-overflow-style:none; }}
    ...
    .alloc-terminal-header {{ margin:4px 0; height:24px; line-height:24px; font-size: 11px; font-weight: 600; color: #e5e7eb; text-transform: uppercase; letter-spacing: 0.03em; flex-shrink:0; }}
    .alloc-row {{ display:flex; align-items:stretch; margin:0; font-size:11px; }}
    /* 좌측 고정 라벨 열에서만 사용. sticky 대신 별도 열로 분리됨. */
    .alloc-row-label {{ height:40px; line-height:40px; margin:4px 0; color:#e5e7eb; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; flex-shrink:0; }}
    .alloc-row-track {{ flex:1; position:relative; height:40px; margin:4px 0; border-radius:6px; background:transparent; border:none; overflow:visible; z-index:1; }}
    ...
    .alloc-flight-ovlp-badge {{ position:absolute; top:2px; right:4px; font-size:9px; font-weight:700; padding:1px 4px; border-radius:3px; background:#e879f9; color:#0f172a; pointer-events:none; }}
    ...
    /* EIBT/EOBT 보조 바차트: E계열 길이대로 행 기준 배치, 파란 바와 동일 두께( top/bottom 4px ), 선택 바 위에도 표시 */
    .alloc-e-bar {{
      position:absolute;
      top:4px;
      bottom:4px;
    ...
    .alloc-time-dot {{ position:absolute; top:50%; transform:translate(-50%,-50%); width:6px; height:6px; border-radius:50%; pointer-events:none; z-index:4; }}
    .alloc-time-dot-s {{ background:#007aff !important; }}
    .alloc-time-dot-sd {{ background:#f97316; }}
    .alloc-time-dot-e {{ background:#fb37c5; }}
...
<body>
        ...
        <div id="tab-allocation" class="tab-content">
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
            ...
            <div style="display:flex;align-items:center;gap:8px;">
              ...
              <span style="width:1px;height:12px;background:#444;margin:0 4px;"></span>
              <label style="font-size:11px;color:#9ca3af;display:flex;align-items:center;gap:4px;margin:0;">
                <input type="checkbox" id="chkShowSDots" />
                SLDT, STOT
              </label>
              <label style="font-size:11px;color:#9ca3af;display:flex;align-items:center;gap:4px;margin:0;">
                <input type="checkbox" id="chkShowSdDots" />
                SLDT(d), STOT(d)
              </label>
              <label style="font-size:11px;color:#9ca3af;display:flex;align-items:center;gap:4px;margin:0;">
                <input type="checkbox" id="chkShowEDots" />
                ELDT, ETOT
              </label>
            </div>
          </div>
  ...
  (function() {{
    ...
    ['chkShowSDots','chkShowSdDots','chkShowEDots'].forEach(id => {{
      const el = document.getElementById(id);
      if (el) el.addEventListener('change', function() {{ if (typeof renderFlightGantt === 'function') renderFlightGantt(); }});
    }});
    ...
    function renderFlightList(skipAutoAllocate) {{
      ...
      const dataRows = flightsSorted.map(f => {{
        ...
        const stotOrig = tDepMin + vttDepMin;
        const sldtStr = formatMinutesToHHMMSS(sldtOrig);
        ...
        return '' +
          '<tr class="flight-data-row obj-item" data-id="' + f.id + '">' +
            ...
            '<td class="flight-td-time flight-col-s">' + stotStr + '</td>' +
            ...
            '<td class="flight-td-time flight-col-sd">' + stotStr_d + '</td>' +
    ...
    function renderFlightGantt() {{
      ...
      const intervals = flights.map(f => {{
        ...
        const timeMin = f.timeMin != null ? f.timeMin : 0;
        const dwell = f.dwellMin != null ? f.dwellMin : 0;
        const v = Math.max(1, f.velocity != null ? f.velocity : 15);
        const arrPts = (typeof getPathForFlight === 'function') ? getPathForFlight(f) : null;
        if (arrPts && arrPts.length >= 2) {{
        ...
        const sldtOrig = Math.max(0, timeMin - vttArrMin);
        const stotOrig = timeMin + dwell + vttDepMin;
        return {{ f, t0, t1, sldt, stot, eibt, eobt, eldt, etot, sldtOrig, stotOrig }};
      }});
      ...
      function buildRowHtml(label, standId) {{
        ...
        const showSDotsEl = document.getElementById('chkShowSDots');
        const showSDots = showSDotsEl && showSDotsEl.checked;
        const showSdDotsEl = document.getElementById('chkShowSdDots');
        const showSdDots = showSdDotsEl && showSdDotsEl.checked;
        const showEDotsEl = document.getElementById('chkShowEDots');
        const showEDots = showEDotsEl && showEDotsEl.checked;
        ...
        const blocks = rowFlights.map(it => {{
          ...
          if (showSDots && sDots) {{
            pushDot(sDots, it.sldtOrig, 'alloc-time-dot-s');
            pushDot(sDots, it.stotOrig, 'alloc-time-dot-s');
          }}
      ...
      order.forEach(key => {{
        ...
        labelRows.push('<div class="alloc-terminal-header">' + escapeHtml(headerLabel) + '</div>');
    ...
    function computeScheduledDisplayTimes(flights) {{
      ...
      Object.keys(standToFlights).forEach(standId => {{
        ...
        list.forEach(f => {{
          ...
          const overlap = Math.max(0, prevSOBT - (f.sibtMin_d != null ? f.sibtMin_d : 0));
          f.vttADelayMin = overlap;
          f.sibtMin_d = (f.sibtMin_d != null ? f.sibtMin_d : 0) + overlap;
          const effectiveDwell = Math.max(f.dwellMin != null ? f.dwellMin : 20, f.minDwellMin != null ? f.minDwellMin : 20);
          f.sobtMin_d = f.sibtMin_d + effectiveDwell;
    ...
    function computeSeparationAdjustedTimes() {{
      ...
      if (typeof computeScheduledDisplayTimes === 'function') computeScheduledDisplayTimes(flights);
      ...
      runways.forEach(rwy => {{
        ...
        flights.forEach((f, flightIdx) => {{
          ...
          if (depRwy === rwy.id) events.push({{ time: stotMin_d, type: 'dep', flight: f, cat: cat, vttDepMin, vttArrMin, sobtMin: sobtMin_d, index: eventIndex++ }});
        }});
      ...
      Object.keys(standToFlightsE).forEach(standId => {{
        ...
        list.forEach(f => {{
          ...
          const effectiveDwell = Math.max(f.dwellMin != null ? f.dwellMin : 20, f.minDwellMin != null ? f.minDwellMin : 20);
          f.eobtMin = f.eibtMin + effectiveDwell;
          f.etotMin = f.eobtMin + vttDepMin;
